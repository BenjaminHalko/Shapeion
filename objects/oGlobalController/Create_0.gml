...
if os_type == os_android {
	function Settings(_name,_num) constructor {
		...
		step = function() {
			...
			if num != 0 {
				percent = ApproachFade(percent,!cooldown and point_in_rectangle(mouse_x,mouse_y,x-width/2,y-height/2,x+width/2,y+height/2) and !oGlobalController.disableSettingsButton,0.035,0.7);	
		
				if mouse_check_button_pressed(mb_left) or !point_in_rectangle(mouse_x,mouse_y,x-width/2,y-height/2,x+width/2,y+height/2) cooldown = false;
				...
				if percent == 1 {
					if num == 1 {
						...
						url_open("https://benjamin.halko.ca/google-play/shapeion/privacypolicy.html");	
					}
					cooldown = true;
				}
			} else {
				if !oGlobalController.volClick {
					if mouse_check_button_pressed(mb_left){
						if point_in_rectangle(mouse_x,mouse_y,x-width/2,y-height/2,x+width/2,y+height/2) {
							oGlobalController.volClick = true;
							oGlobalController.disableSettingsButton = true;
						} else oGlobalController.disableSettingsButton = false;
					}
				} else {
					...
					if !mouse_check_button(mb_left)	oGlobalController.volClick = false;
				}
